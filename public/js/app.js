...
function naturalBlackjack(i) {
  ...
  players[i].balance =
    ...
    players[i].win = (1.5 * players[i].bet + players[i].bet);
  // players[i].bet = 0;
}
...
function finalCompareGo() {
  ...
  if (theClient.sum > 21) {
    ...
    $("#player-result-big-answer").text("DRAW");
     $("#player-result-big-sum").text(doCurrency(theClient.bet))
     $("#player-result-big-plus-minus").text("+")
  } else {
    // youLose.play();
