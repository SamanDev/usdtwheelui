...
let _auth = [_token, _username];
let ws = new WebSocket(HOST, _auth);
...
leaveTable.addEventListener("click", (e) => {
    ...
    theClient.bet = 0;
    $("#total-bet").text(doCurrency(theClient.bet));
    $("#balance").text(doCurrency(theClient.balance));
...
function clientsetuserBalance(data) {
    var theCli = theClient;
    theCli.balance = data.balance;
    const payLoad = {
...
window.addEventListener("load", (event) => {
    ...
    window.addEventListener("message", function (event) {
        if (event?.data?.username) {
            clientsetuserBalance(event.data);
        }
    });
...
ws.onmessage = (message) => {
    ...
    // connect
    if (response.method === "tables") {
        const games = response.games;
console.log(games.length)
        if (games.length) {
            for (let i = 0; i < games.length; i++) {
                $("#main-box").append(`<div><button onclick="goto('` + games[i].id + `')" class="play-btns">Join ` + games[i].players.length + `/7 Players<br/>` + games[i].id + `</button></div>`);
            }
        }
    }
            ...
            for (let i = 0; i < playerSlotHTML.length; i++) {
                for (let x = 0; x < spectators.length; x++) {
                    if (spectators[x].clientId === playerSlotHTML[i]) {
                        ...
                        playerSlot[z].firstElementChild.nextElementSibling.innerHTML += `<span><img class="player-avatar" src="/imgs/avatars/` + spectators[x].avatar + `.webp" alt="avatar"></span>`;
                    }
                }
            }
        }, 50);
        ...
        for (let i = 0; i < playerSlotHTML.length; i++) {
            for (let x = 0; x < players.length; x++) {
                if (players[x].clientId === playerSlotHTML[i]) {
                    ...
                    playerSlot[z].firstElementChild.nextElementSibling.innerHTML += `<span><img class="player-avatar" src="/imgs/avatars/` + players[x].avatar + `.webp" alt="avatar"></span>`;
                }
            }
        }
            ...
            for (let d = 0; d < deckImg.length; d++) {
                for (let c = 0; c < dealer.cards.length; c++) {
                    if (dealer.cards[c].suit + dealer.cards[c].value.card === deckImg[d]) {
                        dealerSlot.lastElementChild.firstElementChild.innerHTML += `<img class="dealerCardImg" src="/imgs/` + deckImg[d] + `.svg">`;
                    }
                }
            }
        }
    ...
    if (response.method === "updatePlayerCards") {
        ...
        for (let i = 0; i < playerSlotHTML.length; i++) {
            if (player.clientId === playerSlotHTML[i]) {
                ...
                // Animation
                setTimeout(function () {
                    $(".players:eq(" + playerSlotHTML.indexOf(playerSlotHTML[i]) + ") .player-cards")
                        .children()
        ...
        for (let i = 0; i < playerSlotHTML.length; i++) {
            for (let x = 0; x < players.length; x++) {
                if (players[x].clientId === playerSlotHTML[i]) {
                    ...
                    playerSlot[z].firstElementChild.nextElementSibling.nextElementSibling.innerHTML += `<span><img class="player-avatar" src="/imgs/avatars/` + players[x].avatar + `.webp" alt="avatar"></span>`;
                }
            }
        }
    }
...
function updateAllPlayers() {
    ...
    if (theClient.blackjack === false) $("#balance").text(doCurrency(theClient.balance));
}
    ...
    for (let s = 0; s < playerSlotHTML.length; s++) {
        for (let i = 0; i < players.length; i++) {
            if (players[i].isReady && players[i].clientId === playerSlotHTML[s]) {
                ...
                if (players[i].bet >= 50000 && players[i].bet < 100000) {
                    chipIndex = "White";
                } else if (players[i].bet >= 100000 && players[i].bet < 150000) {
                    chipIndex = "Green";
                } else if (players[i].bet >= 150000 && players[i].bet < 200000) {
                    chipIndex = "Gray";
                } else if (players[i].bet >= 200000 && players[i].bet < 500000) {
                    chipIndex = "Purple";
                } else if (players[i].bet >= 500000 && players[i].bet < 1000000) {
                    chipIndex = "Orange";
                } else if (players[i].bet >= 1000000 && players[i].bet < 2000000) {
                    chipIndex = "Red";
                } else if (players[i].bet >= 2000000 && players[i].bet < 3000000) {
                    chipIndex = "Blue";
                } else if (players[i].bet >= 3000000 && players[i].bet < 5000000) {
                    chipIndex = "Brown";
                } else if (players[i].bet >= 5000000) {
                    chipIndex = "Black";
                }
